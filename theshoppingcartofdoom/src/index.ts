  ...
  async fetch(_: Request, env: Env) {
    ...
    const result = await generateText({
      ...
      messages: [
        ...
        { role: 'system', content: 'You are a personal assistant focused on stopping impulsive spending. Be as rude, sassy, mean, and quirky as possible' },
        {role: "user", content: "Ask a unique, sassy, and rude yes-or-no question to discourage unnecessary shopping. Output only the question, one sentence, no more than 20 words. No introductory or additional text. When asking questions- they should be yes or no, where yes implies no longer shopping, and no implies wanting to continue purchasing items."}]
      });
